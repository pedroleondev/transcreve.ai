# legado-landing-n8n.md — Conteúdo Preservado (README antigo)

> Este documento **não descreve o TranscreveAI**. É o `README.md` original deste repositório, de um projeto anterior — uma landing page de cotação de plano de saúde integrada a n8n/PostgreSQL/EvolutionAPI — preservado aqui por histórico ao corrigir o README da raiz (T-10 em [TASKS.md](TASKS.md)).
>
> `config.js` (na raiz do repo) ainda contém as variáveis `N8N_WEBHOOK_URL`/`REDIRECT_TO_WHATSAPP` descritas abaixo, mas **não é referenciado por nenhum arquivo do TranscreveAI** (`server.js`, `app.js`, `index.html` atual) — é código órfão desse projeto anterior. Se confirmado que não há mais uso, é candidato a remoção.
...
## Arquivos Criados
1. **`index.html`**: O site contendo o formulário de 4 etapas construído com Tailwind CSS e Alpine.js.
2. **`config.js`**: Arquivo de configuração onde você define a URL do Webhook do seu n8n e outras opções (como números de contato do WhatsApp).
...
Abra o arquivo `config.js` e altere as variáveis:
