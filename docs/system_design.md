...
### 7. JEV: juiz de validação entre a IA e o usuário (T-24/T-25)
**Decisão:** o aprimoramento passou a ter **dois modelos em sequência**: o gerador (`analysis_model`) produz o texto; o juiz (`judge_model`, também OpenRouter) avalia cada chunk antes da entrega (fidelidade, glossário, estrutura, idioma — veredicto estrito em JSON). Reprovação dispara 1 retry do chunk com o feedback do juiz; persistindo a reprovação, entrega com aviso (nunca texto inventado). Modelos e ativação configuráveis no painel admin (`judge_model`, `judge_enabled`); veredicto auditável em `ai_analyses.judge_*`.
**Por quê:** permite trocar o gerador por um modelo mais barato sem cair a qualidade percebida — o juiz barato segura a porta. É o diferencial de mercado do dono (mercado migrando para arquiteturas com juiz). Custo medido em produção: 913 tokens in / 83 out num texto de 317 chars (gerador + juiz).
**Tradeoff aceito:** cada chunk paga ~1 chamada extra de juiz (input = original + candidato, output mínimo). Em textos curtos o overhead pesa; em áudios longos (o caso real) é proporcionalmente pequeno. Veredicto inválido do juiz (JSON quebrado) segue como aprovado com log — não se trava o usuário por falha do juiz.
**Quando revisitar:** se a taxa de reprovação em produção ficar alta (medir via `judge_approved`), avaliar juiz por resultado-final vs. por chunk; avaliar JEV também em `/api/chat` e `/api/translate`.
